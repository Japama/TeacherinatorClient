...
interface ClassroomTypeFormProps {
    ...
    onCreate: (classroomtype: ClassroomType) => void;
    classroomtype: ClassroomType;
    classroomtypes: ClassroomType[];
}

function ClassroomTypeForm(props: ClassroomTypeFormProps) {
    const { isOpen, onClose, onEdit, onCreate, classroomtype } = props;
    const [editedClassroomType, setEditedClassroomType] = useState(classroomtype);

    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const updatedClassroomType = new ClassroomType({ ...editedClassroomType, [event.target.name]: event.target.value });
        setEditedClassroomType(updatedClassroomType);
    };

    const handleSubmit = (event: React.FormEvent) => {
        event.preventDefault();
        if (editedClassroomType.id)
            onEdit(editedClassroomType);
        else
            onCreate(editedClassroomType);
        onClose();
    ...
    return (
        <div className="modal" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1000, backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
            <div className="rounded-md modal-content w-full mx-auto lg:w-[500px] drop-shadow-lg bg-white p-12" style={{ position: 'relative', margin: '0 auto', top: '50%', transform: 'translateY(-50%)' }}>
                <h1 className="backdrop-blur-sm text-4xl pb-8">  {classroomtype.id ? 'Editar tipo aula' : 'Crear tipo aula'}</h1>
                <form id="classroomtypeForm" onSubmit={handleSubmit} className="space-y-5">
                    <div className="relative">
                        <label htmlFor="type_name" className="block">Nombre</label>
                        <input id="type_name" type="text" name="type_name" required value={editedClassroomType.type_name} onChange={handleChange} className="text-center rounded-md p-3 block w-full px-10 drop-shadow-lg outline-none" />
                    </div>
                    <button type="submit" form="classroomtypeForm" className="rounded-md py-2 px-5 mb-4 mt-6 shadow-lg before:block before:-left-1 before:-top-1 before:bg-black before:absolute before:h-0 before:w-0 before:hover:w-[100%] before:hover:h-[100%] before:duration-500 before:-z-40 after:block after:-right-1 after:-bottom-1 after:bg-black after:absolute after:h-0 after:w-0 after:hover:w-[100%] after:hover:h-[100%] after:duration-500 after:-z-40 bg-white relative inline-block">
...
export default ClassroomTypeForm;
