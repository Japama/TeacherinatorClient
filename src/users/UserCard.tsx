...
function UserCard(props: UserCardProps) {
    ...
    return (
        ...
        <tr className="h-[70px] border-b bg-gray-100 text-gray-800">
                ...
                <td className="px-6 py-4 text-center text-lg font-bold"> {user.username} </td>
                <td className="px-6 py-4 text-center"> {user.is_admin ? '✅' : '❌'} </td>
                <td className="px-6 py-4 text-center"> {user.department_id !== 1 ? '✅' : '❌'} </td>
                <td className="px-6 py-4 text-center text-lg font-bold"> {user.department !== null ? user.department?.name : '❌'} </td>
                <td className="px-6 py-4 text-center"> {user.active ? '✅' : '❌'} </td>
                <td colSpan={2} className="px-6 py-4 text-center flex">
