...
function ScheduleForm(props: ScheduleFormProps) {
    const { isOpen, isCreate, onClose, onEdit, onCreate, schedule } = props;
    const [editedSchedule, setEditedSchedule] = useState(schedule);

    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const updatedSchedule = new Schedule({ ...editedSchedule, [event.target.name]: event.target.value });
    ...
    return (
        <div className="modal " style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1000, backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
            ...
            <h1 className="backdrop-blur-sm text-4xl pb-8">  {schedule.id ? 'Editar departamento' : 'Crear departamento'}</h1>
                <form id="scheduleForm" onSubmit={handleSubmit} className="space-y-5">
                    <div className="relative">
                        <label htmlFor="name" className="block">Nombre</label>
                        <input id="name" type="text" name="name" required value={editedSchedule.course} onChange={handleChange} className=" text-center rounded-md  p-3 block w-full px-10 drop-shadow-lg outline-none" />
                    </div>
