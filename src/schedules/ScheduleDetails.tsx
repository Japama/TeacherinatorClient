  ...
  return (
    isOpen ? (
      <div className="modal " style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1000, backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
        <div className=" rounded-md modal-content w-full mx-auto lg:w-full drop-shadow-lg bg-white p-12" style={{ position: 'relative', margin: '0 auto', top: '50%', transform: 'translateY(-50%)' }}>
                        ...
                        <React.Fragment key={index}>
                          {index === 0 && (
                            <td className="border px-4 py-2">
                              {hour ? `${formatTime(hour.start_time)} ` : ' '}
                              <br />
                              {hour ? ` ${formatTime(hour.end_time)}` : ' '}
                            </td>
                          )}
