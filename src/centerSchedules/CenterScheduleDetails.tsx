...
const ScheduleDetails = ({ isOpen, onClose, username, group, centerScheduleHours }: ScheduleDetailsProps) => {
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    const title = "Horario del centro ";
  ...
  const hourWeekdayHours: { [key: number]: CenterScheduleHour[] } = {};

  centerScheduleHours.forEach(hour => {
    ...
    isOpen ? (
      <div className="modal flex items-center justify-center" style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', zIndex: 1000, backgroundColor: 'rgba(0, 0, 0, 0.5)' }}>
        <div className="modal-content rounded-lg w-full mx-auto lg:w-4/5 drop-shadow-2xl bg-white p-8 " style={{ position: 'relative' }}>
          <h2 className="backdrop-blur-md text-4xl pb-8 text-center text-blue-600">{`Horario del centro}`}</h2>
